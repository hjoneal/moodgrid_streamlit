...
if PLAYLIST_LINK:
            ...
            for track in tracks:
                if track["track"] is not None:
                    ...
                    if features is not None:
                        ...
                        printing_container.text(f"Successfully read '{playlist_input_name}' - Track {track_counter}/{pl_length}: {track_name} by {artists}")
    ...
    EC_HS_features = joblib.load("../data/pkl/models/22mar_47pl_common_features.pkl")

    # keep the common EC, HS features for the modelling
    df_sel = df[EC_HS_features]

    # load the scalers and models
    EC_scaler = joblib.load("../data/pkl/models/22mar_47pl_EC_scaler.pkl")
    EC_logreg_model = joblib.load("../data/pkl/models/22mar_47pl_EC_logreg_model.pkl")
    HS_scaler = joblib.load("../data/pkl/models/22mar_47pl_HS_scaler.pkl")
    HS_logreg_model = joblib.load("../data/pkl/models/22mar_47pl_HS_logreg_model.pkl")
    ...
    # joblib.dump(df, "../data/pkl/transform_playlist_glasto_df.pkl")

    # df = joblib.load("../data/pkl/transform_playlist_glasto_df.pkl")
    ...
    left_quad = energy_min/2
    right_quad = energy_max/2
    top_quad = happy_max/2
    bottom_quad = happy_min/2
    ...
    fig1 = px.scatter(df, x="energy_coeff", y="happy_coeff",title="Mood Grid", hover_data=["artists", "track_name"], height=800,width=1200,opacity=0.7,
    ...
    fig1.update_layout(annotations=[
            dict(x=left_quad,y=top_quad,xref="x",yref="y",text="Happy & Chilled",showarrow=False,font=dict(size=20)),
            dict(x=right_quad,y=top_quad,xref="x",yref="y",text="Happy & Energetic",showarrow=False,font=dict(size=20)),
            dict(x=left_quad,y=bottom_quad,xref="x",yref="y",text="Sad & Chilled",showarrow=False,font=dict(size=20)),
            dict(x=right_quad,y=bottom_quad,xref="x",yref="y",text="Sad & Energetic",showarrow=False,font=dict(size=20))])
    fig1.update_layout(title={'font': {'size': 25}})
    #fig1.show()
    ...
    if user_submit:
        ...
        # add new playlist
        fig2 = px.scatter(df, x="energy_coeff", y="happy_coeff",title="Mood Grid", color=df["selected_str"].map({"No": playlist_input_name, "Yes": new_playlist_name}),
                        hover_data=["artists", "track_name"],height=800,width=1200,opacity=0.7,
        ...
        fig2.update_layout(annotations=[
                ...
                dict(x=left_quad,y=bottom_quad,xref="x",yref="y",text="Sad & Chilled",showarrow=False,font=dict(size=20)),
                dict(x=right_quad,y=bottom_quad,xref="x",yref="y",text="Sad & Energetic",showarrow=False,font=dict(size=20))])
        fig2.update_layout(title={'font': {'size': 25}})
        #fig.show()
        ...
        playlist_description = f"A playlist subsetted from {playlist_input_name} with a Happy rating of {find_happy} and an Energy rating of {find_energy}"
